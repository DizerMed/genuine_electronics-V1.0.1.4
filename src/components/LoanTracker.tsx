...
import { 
  ...
  Flame, Zap, AlertCircle, PieChart, ListFilter, ArrowUpDown, Stamp
} from 'lucide-react';
...
export const LoanTracker: React.FC<LoanTrackerProps> = ({
  ...
  const handleSubmitRepayment = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedLoanForRepayment || repayAmount <= 0) {
    ...
    if (repayAmount > meta.remainingBalance + 100) {
    ...
    try {
      ...
      const newBalance = Math.max(0, meta.remainingBalance - repayAmount);
      ...
      if ((selectedLoanForRepayment as any)._origin === 'online' && onUpdateOrder) {
        ...
        if (orig) {
          const updatedOrder: Order = {
            ...orig,
            paidAmount: (orig.paidAmount || 0) + repayAmount,
      ...
      showAlert(
        'Repayment Recorded', 
        `Successfully logged installment of ${formatTZS(repayAmount)} for ${selectedLoanForRepayment.customerName || 'Customer'}. Remaining balance: ${formatTZS(newBalance)}.`,
        'alert'
      );
  ...
  const handleSendWhatsAppReminder = (tx: POSTransaction) => {
    triggerHaptic('light');
    const meta = getLoanMeta(tx);
    let rawPhone = getLoanCustomerPhone(tx).replace(/[^0-9+]/g, '');
    if (!rawPhone) {
      const entered = window.prompt("Enter customer WhatsApp phone number (e.g. 255768...):", "255");
      if (!entered) return;
              ...
              >
                ...
                <div className="space-y-2 pt-4 mt-3 border-t border-slate-200 dark:border-slate-800">
                  ...
                  <div className="grid grid-cols-4 gap-1.5 pt-1">
                    ...
                    <button
                      onClick={() => {
                        if (!getLoanCustomerPhone(tx)) {
                          handleOpenEditCustomer(tx);
                        } else {
                          handleSendWhatsAppReminder(tx);
                        }
                      }}
              ...
              <form onSubmit={handleSubmitRepayment} className="space-y-4">
                ...
                <div className={`p-3 rounded-xl border flex justify-between items-center text-xs ${
                  ...
                  <span className="font-black text-sm text-blue-500">
                    {formatTZS(Math.max(0, meta.remainingBalance - (repayAmount || 0)))}
                  </span>
                </div>
                ...
                <div className="flex gap-2.5 pt-2">
                  ...
                  >
                    ...
                    <span>{isSubmittingRepayment ? 'Recording...' : `Confirm & Receive ${formatTZS(repayAmount)}`}</span>
                  </button>
                </div>
              </form>
