...
export const AIChatWidget: React.FC<AIChatWidgetProps> = ({ isOpen, onClose, products, theme = 'dark' }) => {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/ai-chat', {
        ...
        body: JSON.stringify({
          message: userMsg,
          context: 'Client shopping assistance and genuine electronics specs consultation',
          productCatalog: products.map((p) => ({ name: p.name, brand: p.brand, price: p.price, category: p.category, specs: p.specs })),
        }),
      });

      const data = await res.json();
      setMessages((prev) => [...prev, { sender: 'bot', text: data.reply || 'I am here to help you select genuine electronics!' }]);
    } catch (err) {
      setMessages((prev) => [...prev, { sender: 'bot', text: 'Sorry, I encountered an error connecting to the AI assistant. Please try again.' }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-50 w-full max-w-sm sm:max-w-md rounded-3xl shadow-2xl border overflow-hidden flex flex-col h-[500px] transition-colors duration-200 ${
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-bold text-sm flex items-center gap-1">
              VoltBot AI Expert <Sparkles className="w-3.5 h-3.5 text-amber-300" />
            </h3>
            <p className="text-[10px] text-slate-300 flex items-center gap-1">
              <ShieldCheck className="w-3 h-3 text-emerald-400" /> Genuine Electronics Assistant
            </p>
          </div>
        </div>
      ...
      <div className={`flex-1 overflow-y-auto p-4 space-y-3 ${isDark ? 'bg-slate-950' : 'bg-slate-50'}`}>
        ...
        {loading && (
          <div className="flex justify-start">
            <div className={`border rounded-2xl px-4 py-3 text-xs animate-pulse flex items-center gap-1.5 shadow-sm ${
