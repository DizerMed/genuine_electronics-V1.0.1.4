...
function normalizeClientItem<T>(tableName: string, item: any): T {
  ...
  if (isMatchingCollection(tableName, 'orders') || isMatchingCollection(tableName, 'posTransactions') || isMatchingCollection(tableName, 'pos_transactions')) {
    ...
    // Direct mappings
    if (clone.payment_method !== undefined && clone.paymentMethod === undefined) clone.paymentMethod = clone.payment_method;
    if (clone.paymentmethod !== undefined && clone.paymentMethod === undefined) clone.paymentMethod = clone.paymentmethod;
    if (clone.payment_status !== undefined) clone.paymentStatus = clone.payment_status;
    if (clone.paymentstatus !== undefined && clone.paymentStatus === undefined) clone.paymentStatus = clone.paymentstatus;
...
export function useSupabaseAuth() {
  ...
  const syncAuth = useCallback((authUser: any) => {
    if (authUser) {
      ...
      try {
        localStorage.setItem('ge_user_session', JSON.stringify(sessionUser));
      } catch (e) {
