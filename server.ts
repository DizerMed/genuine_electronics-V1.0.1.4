...
function normalizeFromSupabase(colName: string, row: any): any {
  ...
  if (result.payment_method !== undefined && result.payment_method !== null) result.paymentMethod = result.payment_method;
  if (result.paymentmethod !== undefined && result.paymentmethod !== null) result.paymentMethod = result.paymentmethod;
  if (result.payment_status !== undefined && result.payment_status !== null) result.paymentStatus = result.payment_status;
  if (result.paymentstatus !== undefined && result.paymentstatus !== null) result.paymentStatus = result.paymentstatus;
  if (result.tracking_number !== undefined && result.tracking_number !== null) result.trackingNumber = result.tracking_number;
  if (result.trackingnumber !== undefined && result.trackingnumber !== null) result.trackingNumber = result.trackingnumber;
  if (result.courier_name !== undefined && result.courier_name !== null) result.courierName = result.courier_name;
  if (result.couriername !== undefined && result.couriername !== null) result.courierName = result.couriername;
  if (result.estimated_delivery !== undefined && result.estimated_delivery !== null) result.estimatedDelivery = result.estimated_delivery;
  if (result.estimateddelivery !== undefined && result.estimateddelivery !== null) result.estimatedDelivery = result.estimateddelivery;
  if (result.tracking_timeline !== undefined && result.tracking_timeline !== null) result.trackingTimeline = result.tracking_timeline;
  if (result.trackingtimeline !== undefined && result.trackingtimeline !== null) result.trackingTimeline = result.trackingtimeline;
  if (result.cashier_name !== undefined && result.cashier_name !== null) result.cashierName = result.cashier_name;
  if (result.cashiername !== undefined && result.cashiername !== null) result.cashierName = result.cashiername;
  if (result.full_name !== undefined && result.full_name !== null) result.fullName = result.full_name;
  if (result.fullname !== undefined && result.fullname !== null) result.fullName = result.fullname;
  if (result.badge_text !== undefined && result.badge_text !== null) result.badgeText = result.badge_text;
  if (result.badgetext !== undefined && result.badgetext !== null) result.badgeText = result.badgetext;
  if (result.start_date !== undefined && result.start_date !== null) result.startDate = result.start_date;
  if (result.startdate !== undefined && result.startdate !== null) result.startDate = result.startdate;
  if (result.end_date !== undefined && result.end_date !== null) result.endDate = result.end_date;
  if (result.enddate !== undefined && result.enddate !== null) result.endDate = result.enddate;
...
function preparePayloadForSupabase(sqlTable: string, item: any): any {
  ...
  if (payload.paymentStatus || payload.payment_status || sqlTable === 'orders') {
    const pStat = String(payload.paymentStatus || payload.payment_status || 'Pending');
  ...
  if (payload.trackingNumber || payload.tracking_number || sqlTable === 'orders') {
    const tNum = String(payload.trackingNumber || payload.tracking_number || '');
  ...
  if (payload.courierName || payload.courier_name) {
    const cName = String(payload.courierName || payload.courier_name || '');
  ...
  if (payload.estimatedDelivery || payload.estimated_delivery) {
    const eDel = String(payload.estimatedDelivery || payload.estimated_delivery || '');
...
app.post('/api/data/:collection', async (req, res) => {
  try {
    ...
    let itemToSave = { ...item, id: docId, updatedAt: now };
    ...
    const normalized = normalizeFromSupabase(colName, itemToSave);
...
app.put('/api/data/:collection/:id', async (req, res) => {
  try {
    ...
    // Remove stale snake_case / camelCase properties from existing if req.body provides the opposite case
    ...
    const now = new Date().toISOString();
    let itemToSave = { ...cleanExisting, ...req.body, id, updatedAt: now };
    let supabaseSynced = false;
...
app.get('/api/analytics/visitors', (req, res) => {
  try {
    const { productId, interactionType, searchQuery, deviceType, timeframe, startDate, endDate } = req.query;
